...
PaperGlidr uses two R2 buckets:

| Bucket | Env var | Holds | Who can read it |
|---|---|---|---|
| Private (e.g. `paperglidr-private`) | `R2_BUCKET_NAME` | Lesson videos, PDFs and attachments, invoice PDFs, and image uploads waiting to be checked (`image-uploads/`) | Nobody directly. The app hands out short-lived signed URLs after an access check |
| Public (e.g. `paperglidr-images`) | `R2_PUBLIC_BUCKET_NAME` | Product thumbnails (`products/`) and instructor photos (`instructors/`) | Anyone, at `R2_PUBLIC_BASE_URL` (e.g. `https://images.paperglidr.com`) |
...
1. **Private bucket.** R2 → Create bucket → e.g. `paperglidr-private`, location Automatic. Leave **Public access** off. Do not connect a custom domain or enable the `r2.dev` URL.
2. **CORS on the private bucket.** Browsers upload to it directly with presigned PUT URLs.
   ...
   [
     {
       "AllowedOrigins": ["https://paperglidr.com"],
...
4. **Public bucket.** Create a second bucket, e.g. `paperglidr-images`. No CORS is needed, because browsers only GET from it through `<img>` and next/image.
5. **Custom domain for the public bucket.**
   1. Bucket → Settings → Custom Domains → Connect domain → `images.paperglidr.com`. The zone must be on Cloudflare, and Cloudflare creates the DNS record.
   2. Leave the `r2.dev` public URL **disabled**. It is rate-limited and not meant for production.
6. **Optional hardening.**
   1. On the `paperglidr.com` zone, go to Rules → Transform Rules → Modify Response Header.
   2. Match: hostname equals `images.paperglidr.com`.
...
8. **Env vars.** Set these in Vercel for Production (and in Preview with separate dev buckets):
   ...
   R2_BUCKET_NAME=paperglidr-private
   R2_PUBLIC_BUCKET_NAME=paperglidr-images
   R2_PUBLIC_BASE_URL=https://images.paperglidr.com
   NEXT_PUBLIC_IMAGE_HOSTS=images.paperglidr.com
   ```
